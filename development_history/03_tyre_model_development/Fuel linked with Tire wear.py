...
# Lap-time improvement for each kg of fuel burned
fuel_time_per_kg = 0.03

# Extra tyre wear caused by heavy fuel
...
#
# The cliff threshold uses "equivalent wear laps",
# not normal race laps.
#
# =============================================================
...
tyres = {
    ...
    "Soft": (
        89.0,       # Base lap time
        0.10,       # Linear degradation
        0.006,      # Quadratic degradation
        0.30,       # Initial warm-up penalty
        0.15,       # Warm-up recovery
        18.0,       # Cliff threshold
        0.04        # Cliff severity
    ),
...
def calculate_cliff_penalty(
    ...
    # Before the cliff:
    # no extra cliff penalty

    if tyre_wear <= cliff_threshold:
        return 0.0

    # Amount by which the tyre has passed the cliff
    ...
    # Penalty increases rapidly after the cliff
...
def calculate_tyre_loss(
    ...
    # Extra degradation after the cliff
...
for tyre_name in tyres:
    ...
    for tyre_wear in wear_values:
        ...
        penalty = calculate_cliff_penalty(
            tyre_wear,
            tyre_data[5],
            tyre_data[6]
        )

        cliff_penalties.append(
            penalty
        )
...
plt.ylabel(
    "Additional Cliff Penalty (seconds)"
)
